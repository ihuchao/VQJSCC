        ...
        if args.mock_channel and iteration > (args.kmeans_st_iter + 0) and iteration % 10 == 0:
            ...
            with torch.enable_grad():
                ...
                channel_loss, loss_dict = channel_system.compute_loss(
                    ...
                    cont_params,  # target continuous params
                    w_idx=1.0,
                    w_cont=1.0
                )
            ...
            with torch.no_grad():
                ...
                # 立即清理中间变量
            del output, indices, cont_params, R, t, intrinsics
            del corrupted_params  # 已经应用到 gaussians 了
...
def prepare_indices(gaussians, kmeans_rot_q, kmeans_sc_q, kmeans_dc_q, kmeans_sh_q):
    """提取当前的量化索引"""
    indices = torch.stack([
        torch.tensor(kmeans_rot_q.cls_ids, device="cuda"),
        torch.tensor(kmeans_sc_q.cls_ids, device="cuda"),
        torch.tensor(kmeans_dc_q.cls_ids, device="cuda"),
        torch.tensor(kmeans_sh_q.cls_ids, device="cuda")
    ], dim=-1)  # [N, 4]
    return indices
...
def decode_output_to_gaussians(output, gaussians, kmeans_rot_q, kmeans_sc_q, 
    ...
    # ✅ 使用 torch tensor 索引，保持形状
    rot_indices = pred_indices[0].cpu().numpy()
    scale_indices = pred_indices[1].cpu().numpy()
    dc_indices = pred_indices[2].cpu().numpy()
    sh_indices = pred_indices[3].cpu().numpy()
    
    # ✅ 确保形状与原始 gaussians 一致
    corrupted_params['rotation'] = torch.tensor(
        kmeans_rot_q.centers[rot_indices], 
        dtype=torch.float32, 
        device="cuda"
    ).reshape(gaussians._rotation.shape)  # ✅ 明确指定形状
    
    corrupted_params['scale'] = torch.tensor(
        kmeans_sc_q.centers[scale_indices],
        dtype=torch.float32,
        device="cuda"
    ).reshape(gaussians._scaling.shape)  # ✅ 明确指定形状
    
    corrupted_params['f_dc'] = torch.tensor(
        kmeans_dc_q.centers[dc_indices],
        dtype=torch.float32,
        device="cuda"
    ).reshape(gaussians._features_dc.shape)  # ✅ 明确指定形状
    
    corrupted_params['f_rest'] = torch.tensor(
        kmeans_sh_q.centers[sh_indices],
        dtype=torch.float32,
        device="cuda"
    ).reshape(gaussians._features_rest.shape)  # ✅ 明确指定形状
