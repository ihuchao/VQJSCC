...
class GaussianModel:
    ...
    def load_ply(self, path, load_quant=False):
        ...
        quant_params = []
        print("Loading PLY file from", path)
        # Load indices and codebook for quantized params
        if load_quant:
            base_path = '/'.join(path.split('/')[:-1])
            print("Loading quantized parameters from", base_path)
            ...
            # base_path = Path(path).parent  # 自动获取父目录[7,10](@ref)
            # inds_file = base_path / 'kmeans_inds.bin'          # 使用/运算符自动适配系统[10](@ref)
            # codebook_file = base_path / 'kmeans_centers.pth'   # 无需手动拼接
            # args_file = base_path / 'kmeans_args.npy'
            # codebook = torch.load(str(codebook_file))  # Path对象需转字符串[8](@ref)
            # args_dict = np.load(str(args_file), allow_pickle=True).item()
